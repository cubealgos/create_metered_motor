// Create Fly: Metered Motor — one Gradle project, one jar (docs/spec/04-architecture.md ARCH-DEC-001).
plugins {
    java
...
// The pure package: metered_motor.model has no Minecraft imports (docs/spec/operations/testing.md TEST-REQ-002).
val verifyPurePackage by tasks.registering {
    group = "verification"
    description = "Fails when metered_motor.model imports Minecraft, Fabric or Create."
    val sources = layout.projectDirectory.dir("src/main/java/metered_motor/model")
    inputs.dir(sources)
    doLast {
