...
| Area | Position |
|---|---|
| GDPR: what leaves the user's machine | Nothing. No telemetry, no update check, no outbound network call of any kind (`COMP-REQ-001`). No player names in the mod's data; the block entity stores emerald counts, a meter fraction and rolled stats only. |
...
| Server trust boundary | The client never sends stats or emerald counts; every change goes through vanilla container menus and the block entity's sync. The roll runs server-side, inside the loot function, on offer creation. |
