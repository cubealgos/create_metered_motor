---
title: "create_metered_motor DEC-003 — MIT, no CLA, public under cubealgos from the first commit"
type: "spec"
...
the same place `create_brass_compass` ended up, so this project starts there instead of retracing
that path. The commit identity is `scheeren@cubealgos.de`.
...
the compass already made and recorded; this project inherits them rather than deciding them fresh.
Cost if wrong: MIT and a public remote are hard to walk back once someone has forked.
