#!/usr/bin/env python3
"""Render docs/modrinth/icon.png: the motor on the round blueprint badge Create's add-ons share.

The palette is sampled from Create's own Modrinth icon: outer band, pale ring, blueprint blue, white
grid at half alpha. The sprite defaults to docs/modrinth/placeholder-motor.png, a 16x16 stand-in for
the motor's item texture, which does not exist before MM-7; pass a path as the first argument to
render from a different sprite once the real texture lands. Scaled without smoothing, with a
one-pixel white outline and a soft shadow. Requires Pillow.
"""
import sys
...
SHADOW = (20, 50, 90, 130)
SPRITE = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("docs/modrinth/placeholder-motor.png")
