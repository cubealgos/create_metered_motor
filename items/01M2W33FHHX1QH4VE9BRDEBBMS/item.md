...
- [ ] Unit tests: the rate formula at the documented bounds (tier I worst case one emerald per ~20 minutes, tier III best case ~0.8/minute, tier III worst case 3/minute, per `docs/spec/domains/trade.md` §3), the meter's per-second increment under a known stress/capacity ratio, a uniform roll landing inside its band, codec round trip, and a newer `version` read back intact and flagged unknown (`StatsTest`, `RollTest`).
- [ ] `verifyPurePackage` passes on `metered_motor.model` with no Minecraft imports, and a deliberate import proves the check fails (`TEST-REQ-002`, once).
- [ ] The component survives a save round trip through `ItemStack.CODEC` with registry ops in a game test (`StatsComponentGameTest`).
